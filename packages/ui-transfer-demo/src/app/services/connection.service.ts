...
import { getSolidDataset, getStringNoLocale, getThing, getUrlAll, ThingPersisted, getDatetime, getUrl, getThingAll, Thing, createThing, addStringNoLocale, addUrl, addDatetime, saveSolidDatasetAt, SolidDataset, setThing, overwriteFile } from '@inrupt/solid-client';
...
export class ConnectionService {
    ...
    public getName(session: Session): Observable<string> {
        ...
        return of({ session })
            .pipe(
                switchMap(data => from(getSolidDataset(data.session.info.webId, { fetch: data.session.fetch }))
                    .pipe(map(response => ({ ...data, response })))),
                map(data => ({ ...data, profile: getThing(data.response, data.session.info.webId) })),
                map(data => getStringNoLocale(data.profile, 'http://www.w3.org/2006/vcard/ns#fn')),
            );
    }
    public getAllFilesForClass(session: Session, forClass: string): Observable<string[]> {
        ...
        return of({session}).pipe(
            ...
            map(data => ({ ...data, profile: getThing(data.profileResponse, data.session.info.webId) })),
            // handle privateTypeIndex
            map(data => ({ ...data, privateTypeIndex: getUrl(data.profile, this.solid + 'privateTypeIndex')})),
            switchMap(data => from(getSolidDataset(data.privateTypeIndex, { fetch: data.session.fetch }))
                .pipe(map(privateResponse => ({ ...data, privateResponse })))),
            map(data => ({ ...data, privateFoundForClass: getThingAll(data.privateResponse).filter(thing =>
                getUrl(thing, this.solid + 'forClass') === forClass
            ).map(thing => getUrl(thing, this.solid + 'instance'))})),
            // handle publicTypeIndex
            map(data => ({ ...data, publicTypeIndex: getUrl(data.profile, this.solid + 'publicTypeIndex')})),
            switchMap(data => from(getSolidDataset(data.publicTypeIndex, { fetch: data.session.fetch }))
                .pipe(map(publicResponse => ({ ...data, publicResponse })))),
            map(data => ({ ...data, publicFoundForClass: getThingAll(data.publicResponse).filter(thing =>
                getUrl(thing, this.solid + 'forClass') === forClass
            ).map(thing => getUrl(thing, this.solid + 'instance'))})),
            // wrap up
            map(data => [ ...data.privateFoundForClass, ...data.publicFoundForClass ]),
    ...
    public getPaySlips(session: Session): Observable<PaySlip[]> {
        ...
        return of({ session }).pipe(
            switchMap(data => this.getAllFilesForClass(session, this.pay + 'payslips').pipe(
                map(payslipfiles => ({ ...data, payslipfiles }))
            )),
            switchMap(data => data.payslipfiles && data.payslipfiles.length > 0
                ? forkJoin(data.payslipfiles.map(file => this.getAllPaySlipsFromFile(data.session, file)))
                : [[]]),
            map(data => _.flatten(data)),
        ) as Observable<PaySlip[]>;
    }
    private getAllPaySlipsFromFile(session: Session, file: string): Observable<PaySlip[]> {
        if (!session) {
            throw Error('Parameter session should be set');
        }
        if (!file) {
        ...
        return of( {session, file}).pipe(
            switchMap(data => from(getSolidDataset(data.file, { fetch: data.session.fetch }))
                .pipe(map(paySlipResponse => ({ ...data, paySlipResponse })))),
            map(data => ({ ...data, allThings: getThingAll(data.paySlipResponse).filter(thing =>
                getUrl(thing, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type') === this.pay + 'payslip'
            )})),
            map(data => ({ ...data, finalpayslips: data.allThings.map(thing =>
    ...
    private transformOneToDomaion(thing: Thing): PaySlip {
        ...
        const taxableAmount = +getStringNoLocale(thing, this.pay + 'taxable-amount');
    ...
    public saveConsent(session: Session): Observable<Consent> {
        ...
        return of({session}).pipe(
            ...
            switchMap(data => this.getAllFilesForClass(session, this.consent + 'consents').pipe( map( files => ({...data, files}) ) )),
            map(data => ({ ...data, file: data.files.length
                ? data.files[0]
                // assuming webids always end in /profile/card#me
                : data.session.info.webId.slice(0, -16) + this.defaultConsentFile})),
            map(data => ({ ...data, consentThing: this.convertConsentToThing(data.consent) })),
            // Update in pod
            switchMap(data => from(getSolidDataset(data.file, { fetch: data.session.fetch }))
                .pipe(
                    ...
                    catchError( () => {
                        ...
                        return from(saveSolidDatasetAt(data.file, createThing(), { fetch: data.session.fetch })).pipe(
                            switchMap( () => from(getSolidDataset(data.file, { fetch: data.session.fetch }))),
                            map( (dataset: SolidDataset) => ({ ...data, dataset })),
                        );
                    }),
                )),
            map(data => ({ ...data, updatedDataSet: setThing(data.dataset, data.consentThing)})),
            switchMap(data => from(saveSolidDatasetAt(data.file, data.updatedDataSet, { fetch: data.session.fetch }))
                .pipe( map(() => data ) )),
    ...
    private convertConsentToThing(consent: Consent): Thing {
        ...
        thing = addUrl(thing, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', this.consent + 'consent');
