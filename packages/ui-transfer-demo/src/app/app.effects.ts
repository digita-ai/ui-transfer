...
export class AppEffects {
    ...
    connectProvider$ = this.actions$.pipe(
        ...
        switchMap((action) => this.service.connect(action.session, action.provider)),
    );

    @Effect()
    loadProfile$ = this.actions$.pipe(
        ofType(actions.loadProfile),
        switchMap((action) => this.service.getName(action.session).pipe(
            map(name => ({ action, name }))
        )),
        switchMap(data => this.service.getPaySlips(data.action.session).pipe(
            map(payslips => ({ ...data, payslips })),
        )),
        switchMap(data => of( actions.loadProfileFinished( { name: data.name, payslips: data.payslips, session: data.action.session } ) )),
    );
